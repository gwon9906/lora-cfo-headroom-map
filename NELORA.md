...
>
> ⚠️ **3차 개정판 · 4차 검토 반영.** 측정 정정 **열두 건**(그중 둘은 결론 철회) +
...
닿는다. 세 팔 모두 그 격자로 다시 쟀다 — 고전 팔은 `nelora_split.py` 로 held-out 을
복원해 로컬에서(`ext_sf7_plain.json`), DNN 팔은 같은 체크포인트로 WSL 에서
(`ext_sf7_dnn.json`). **겹치는 15개 SNR 이 원래 측정과 0.1%p 이내로 일치**하고 DNN
교차점도 −14.35 dB 로 같다.*
...
문서가 어긋나게 적고 있던 것이고, 그래서 위 열두 건과 번호를 섞지 않는다.
