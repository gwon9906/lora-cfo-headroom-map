...
> `decode_loraphy` −12.98 dB → 그들 신경망 −14.35 dB(**+1.36**, macro +1.45) →
> 학습도 정렬도 없는 표준 수신기 **−16.31 dB(+3.32)**.
> 상한인 D3(정렬된 경험적 MF)는 −17.05 dB 다.
>
> **표준 수신기가 신경망을 +1.60 ~ +2.05 dB 앞선다** — 체크포인트(최종/검증 최고)와
> 모드(train/eval)를 **어떻게 고르든** 그렇다(§5-11 표). 그 중 **그들 조건 그대로**인 값이
> **+1.96 dB** [95% CI +1.31, +2.59] 이고(최종 체크포인트 · train 모드 — 그들 `test()` 가
> `.eval()` 을 부르지 않는다, §5-5), 같은 비교를 macro 축으로 재면 +2.05 dB 다.
> 범위의 **최솟값 +1.60 은 eval 모드 · 검증 최고 에포크**, 곧 DNN 에 가장 유리한 조합이다.
>
> 그 신경망은 그들 코드(시드·패킷분할 패치판)로 150에포크 이어 학습한 것이고, held-out
> macro 이득 **+1.45 dB** 로 그들이 발표한 SF7 수치(+1.46 dB) **수준에 도달한다**
> (정확한 대응은 아니다 — §5-3). 전량 평가 · 별도 체크포인트에서는 +1.50 dB 다(§5-6).
...
>
> ⚠️ **3차 개정판 · 4차 검토 반영.** 측정 정정 **열세 건**(그중 둘은 결론 철회) +
> 문서 표기·논리 **열 건**(§9-11, 측정값 변화 없음).
...
| 그 격차가 정말 대역 밖 잡음인가 | **대부분 설명된다.** 모두에게 대역제한 입력을 주면 **+3.33 → +0.28 dB [−0.14, +0.70]**. 0 과 구별되지 않지만 **잔차 ≤0.7 dB 와도 양립**한다 (§5-10) |
| 데시메이션 + DNN 이 데시메이션 단독을 넘는가 | **못 넘는다.** 대역제한 입력 DNN 은 표준 RX 에 0.82 dB, 자기 baseline 에 0.53 dB 못 미친다 — **점추정, CI 미산출** (§5-11) |
| **같은 배치에서 DNN 과 비교** | **확정.** 표준 RX − DNN = **+1.60 ~ +2.05 dB**(체크포인트·모드 전부), 그들 조건에서 **+1.96** [+1.31, +2.59], 패킷 분리 held-out (§5-4) |
| 그 DNN 이 제대로 학습된 것인가 | **재학습함.** held-out 에서 +1.36 dB (macro **+1.45**) = 그들 발표 +1.46 수준 (§5-4). 전량 평가·다른 체크포인트로는 +1.50 (§5-6) |
...
>
> 학습도 정렬도 없는 한 단계가 신경망보다 **1.6 ~ 2.0 dB** 더 번다.
>
...
*축은 **그들 그림과 같게** 둔다 — 선형 y(0~1), x 는 −10 dB 까지. 왼쪽 끝은 **측정한 곳
까지**(−34 dB)다 — 축만 늘려 놓으면 선이 끊긴 것처럼 보인다. 재검증이므로 독자가 두 그림을
겹쳐 볼 수 있어야 한다. 대신 **읽는 방법**을 고쳤다: 색 있는 수직선이 각 팔의 10% 교차점이고
값을 그대로 적었다(이 보고서의 결론이 그 네 숫자다), 선 스타일을 색과 함께 다르게 주어
흑백 인쇄에서도 구분되며, 주인공인 표준 RX 만 굵다. 연한 점선 둘은 **10% 임계**와
**랜덤 추측 1−1/128 = 99.2%** 다 — 기준선이 데이터보다 강해지지 않게 연하게 둔다.*

*한 번 semilogy 로 바꿨다가 되돌렸다. 이 보고서의 주장은 **10% 교차점 하나**라 로그축의
장점(저SER 구간 확대)을 쓰지 않고, 2페이지 원고에서 작게 들어갈 때 선형이 축소에 강하다.*

*`Empirical MF (aligned)` 가 상한인 근거는 그림이 아니라 정리다 — AWGN 에서 등에너지·
근사직교 신호 집합의 최적 검출은 프로토타입 상관이고 그 상관 벡터가 **충분통계량**이다
(본편 README §4.2, Vangelista [1]). D3 는 그 상관을 **캡처에서 추정한 프로토타입**으로
계산하고 타이밍·CFO 까지 맞춰 준 것이라, 이 데이터에서 도달 가능한 상한에 해당한다.
그래서 그림 라벨에는 `ceiling` 을 쓰지 않는다.*

*세 팔 모두 −34 dB 격자로 다시 쟀다 — 고전 팔은 `nelora_split.py` 로 held-out 을 복원해
로컬에서(`ext_sf7_plain.json`), DNN 팔은 같은 체크포인트로 WSL 에서(`ext_sf7_dnn.json`).
겹치는 15개 SNR 이 원래 측정과 일치하므로, 확장분이 같은 모집단·같은 체크포인트라는
확인도 겸한다.*

*포화까지 그리니 두 가지가 더 보인다 — **표준 RX 가 전 구간에서 DNN 아래**에 있고,
**−28 dB 아래에서는 DNN 이 baseline 으로 수렴한다**(94.5% 대 94.9%). 디노이저가 더 벌
것이 없어지는 지점이다.*
...
### held-out 판 — §0 헤드라인의 숫자는 이것이다

위 표는 그들 `load_data()` **전량**(3000심볼 부분표집)에 **공개 체크포인트를 이어 학습한
`ckpt_sf7`**(§5-6)을 쓴다. 헤드라인은 다르다 — **패킷 분리 held-out 18패킷 / 1353심볼**,
**`ckpt_sf7_plain` 최종 체크포인트**(시드·패킷분할 패치판으로 150에포크), 잡음 실현 10회,
부트스트랩 1000복제. 모집단도 체크포인트도 다르므로 두 표를 섞어 읽으면 안 된다.

| 검출기 (10% SER) | micro | macro |
|---|---|---|
| `decode_loraphy` (U=100) | −12.98 | −12.70 |
| **NELoRa DNN** (`ckpt_sf7_plain` 최종, train 모드) | **−14.35 (+1.36)** | **−14.15 (+1.45)** |
| **표준 RX** (학습·정렬 없음) | **−16.31 (+3.32)** | **−16.20 (+3.50)** |
| D2 이상적 MF (정렬) | −16.96 | −16.82 |
| D3 경험적 MF (정렬) | −17.05 | −16.94 |

패킷 부트스트랩 1000복제(95% CI):

```
표준 RX − DNN          +1.97 dB  [+1.31, +2.59]
D3      − DNN          +2.74 dB  [+2.15, +3.31]
DNN     − baseline     +1.35 dB  [+0.70, +1.97]
표준 RX − baseline     +3.33 dB  [+2.67, +4.02]
```

복제 **중앙값**이다. 위 표의 교차점에서 직접 뺀 점추정은 +1.96 / +2.71 / +1.36 / +3.32 로
0.04 dB 안에서 같다. 본문은 점추정을 쓴다.

같은 held-out 을 **검증 최고 에포크(ep77)** 로 바꾸면 DNN 이 −14.26, **eval 모드**로
바꾸면 −14.71 이다(§5-11 표). 그래서 §0 은 표준 RX 의 우위를 한 값이 아니라
**+1.60 ~ +2.05 dB 범위**로 적는다. 이 표의 +1.96 은 그 중 **그들 조건 그대로**인 값이다.
...
엄밀히는 **0 이 됐다가 아니라 0 과 구별되지 않는다** — CI 상단이 +0.70 dB 이므로
잔차 0.7 dB 와도 양립한다. §6 에서 표준RX−D2 잔차를 "완전히 설명된다고까지는 쓰지
않는다" 로 둔 것과 같은 기준이다. 격차의 **대부분**이 대역 밖 잡음으로 설명된다.
...
측정했다 — 오른쪽 패널은 두 곡선이 포화 구간까지 겹쳐 간다. 축 규약은 그림 1 과 같다
(선형 y, 연한 점선 둘 = 10% 임계·랜덤 추측, 수직선 = 10% 교차점). 오른쪽 패널의 두
교차점이 0.29 dB 안으로 모인다.*
...
표는 **전부 micro** 다. 두 가지를 분명히 해 둔다.

- **헤드라인(§5-4)의 DNN −14.35 와 이 표의 −14.26 은 같은 조건이 아니다.** 둘 다 같은
  학습(`ckpt_sf7_plain`, 시드·패킷분할 패치판으로 150에포크)에서 나왔지만, 헤드라인은
  **최종 체크포인트**이고 이 표는 **검증 최고 에포크(ep77)** 다. 차이 **0.09 dB**.
  (§5-6 의 +1.39/+1.50 은 또 다른 체크포인트 — 그들 `train()` 으로 100에포크 이어 학습한
  `ckpt_sf7` 를 **전량 평가**한 값이다. 셋을 섞어 읽지 말 것.)
- **'필터 없음 / train' 의 +2.05 는 헤드라인의 macro +2.05 와 우연히 같은 값이다.**
  이쪽은 micro · 검증 최고 에포크(+2.048), 저쪽은 macro · 최종 체크포인트(+2.048).
  계산이 다른 두 양이 소수 둘째 자리에서 겹쳤을 뿐이다.
...
| 대역제한 | **−0.53 dB** (eval, DNN 최선) / −1.81 dB (train) — 둘 다 DNN 이 뒤짐 |
...
- **eval / train 중 나은 쪽을 DNN 의 값으로 쓴다.** (헤드라인 §5-4 는 이 원칙을 쓰지 않고
  **그들 조건인 train 모드**를 쓴다 — 그들 `test()` 가 그렇게 돌기 때문이다. 그래서 §0 은
  한 값이 아니라 **+1.60 ~ +2.05 dB 범위**로 쓴다.)

세 가지 모두 DNN 에 유리한 방향이고, 그래도 진다.

#### 반대로 DNN 에 불리한 항목 하나

대역제한 팔은 **필터 없는 입력에 맞춰진 공개 체크포인트에서 이어 학습**한 것이고,
`--scratch` 대조가 없다. §5-9 에서 결과를 보기 전에 **미리 위험으로 지목한 바로 그
항목**이고 §8 에 한계로 남아 있다. 위 수렴표(400에포크 고원, ep144→ep398 이 +0.001)가
상당 부분 방어하지만, 처음부터 학습하면 더 간다는 가능성을 **측정으로 배제하지는
못했다.** 이 한 가지는 결론과 반대 방향이므로 같이 적어 둔다.
...
1. **SNR 축이 합성 AWGN 이다.** 그들도 그렇다(§1-3). 캡처가 이미 잡음을 포함하므로
   실제 SNR 은 표기값보다 낮다. **절대 dB 를 물리적 SNR 로 해석하지 말 것** — 헤드라인이
   −12.98 / −14.35 / −16.31 을 함께 적는 것은 같은 축 위의 위치를 보이기 위해서고,
   결론은 그 **차이**로만 말한다.
...
**열세 건을 정정했고 그중 둘은 결론을 철회했다.** 전부 같은 자리에서 나왔다 —
**내가 재구현한 쪽**이다.
...
"이런 선택을 했고 그 대가가 이만큼이다" 로 쓴다. 반면 아래 열세 건은 그들 결과를
...
| 13 | 확장 측정에 `--batch 16` 을 빼먹음 → 교차점 0.2~0.3 dB 어긋남 (**재발**) | §9-6 | 4차 |
...
**(c) 그리고 4차 검토에서 같은 실수를 다시 했다.** 곡선을 −34 dB 까지 늘리려고 고전 팔을
다시 잴 때 `--batch 16` 을 빼먹었다(기본값 **64**). 잡음 공유 단위가 네 배가 되니 곡선이
통째로 1%p 가량 나빠지고 **교차점이 0.2~0.3 dB 어긋났다** — 그림에 교차점을 적기 시작하자
드러났다(−12.78 / −16.00 이 나왔는데 보고서 값은 −12.98 / −16.31 이다). `--batch 16` 로
다시 돌리니 겹치는 15개 SNR 이 저장본과 **소수 첫째 자리까지 일치**한다.
규약이 코드 밖(명령줄 기본값)에 있으면 같은 실수가 재발한다는 사례다.
...
지우면 `mean|x|` 가 크게 바뀌므로 순서가 결정적이다. 정정 **#6**(그들 `add_noise` 의 정규화 누락 — 본문은 §5-4)과 **같은 자리**다.
...
### 9-11. 문서 자체의 불일치 (4차 검토) — 측정값은 변하지 않았다

검토에서 **표기 6건 · 논리 4건**이 나왔다. 측정을 다시 한 것이 아니라 같은 측정을
문서가 어긋나게 적고 있던 것이고, 그래서 위 열세 건과 번호를 섞지 않는다.

| 무엇 | 어떻게 고쳤나 |
|---|---|
| 헤드라인 −14.35 와 §5-11 −14.26 이 같은 이름의 조건인데 값이 달랐다 | 체크포인트가 다르다(최종 vs 검증 최고 ep77, 0.09 dB). §5-11 에 각주 |
| 헤드라인 네 숫자가 **본문 어느 표에도 없었다** (§5-4 는 전량 3000심볼 표다) | §5-4 에 **held-out 표**를 넣었다 |
| macro 이득이 +1.50 과 +1.45 로 갈렸다 | 전량(§5-6, 다른 체크포인트) vs held-out(§5-4). 둘 다 출처를 붙였다 |
| **+2.05 가 두 뜻**으로 쓰였다 (헤드라인 macro / §5-11 micro) | 우연히 같은 값임을 §5-11 에 명시 |
| "+1.25 ~ +1.75" 는 실제 +1.28 ~ +1.73 을 바깥으로 벌린 반올림 | 실측값으로 |
| §9-8 이 정규화 누락을 §9-2 로 참조 (§9-2 는 alias 철회) | 정정 #6 · 본문 §5-4 로 |

논리 쪽으로는 세 가지를 고쳤다 — ① 귀무 결과(+0.28 [−0.14, +0.70])를 "확정" 으로
쓰던 것을 **"대부분 설명된다(잔차 ≤0.7 dB)"** 로, ② 요약표의 −0.53 dB 에 **CI 미산출**
표시, ③ §5-11 의 "DNN 에 유리하게 기울었다" 목록에 **반대 방향 항목**(대역제한 팔이
이어 학습이고 `--scratch` 대조 없음)을 추가. 그리고 헤드라인이 train 모드를 쓰는데
§5-11 은 "나은 쪽" 원칙을 선언해 **원칙이 절마다 달라 보이던 것**을, 헤드라인을
**+1.60 ~ +2.05 dB 범위**로 바꿔 해소했다. ④ §8 한계1 의 "절대 dB 를 인용하지 말 것" 은
헤드라인이 절대 dB 넷으로 시작하는 것과 충돌해서, **"물리적 SNR 로 해석하지 말 것"** 으로
뜻을 정확히 했다.
...
python nelora_dnn_eval.py --sf 7 --batch 16 --data-dir ../NeLoRa_Dataset/7 \
...
#   `--batch 16` 은 선택이 아니다 — 잡음이 배치 단위로 공유되므로(§9-6) 기본값 64 로
#   돌리면 교차점이 0.2~0.3 dB 어긋난다.
