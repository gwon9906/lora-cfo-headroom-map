...
*y 축은 **로그**다 — 판정이 일어나는 SER 0~20% 구간이 선형축에서는 아래 1/5 로 눌려,
표준 RX 와 D3 의 0.74 dB 차이가 보이지 않았다. x 축은 SER 이 포화하는 −34 dB 까지이고,
연한 점선 둘이 **10% 임계**와 **랜덤 추측 1−1/128 = 99.2%** 다. 색 있는 수직선이 각 팔의
10% 교차점이고 값을 그대로 적었다 — 이 보고서의 결론이 그 네 숫자다.*
...
(로그 y, 연한 점선 둘 = 10% 임계·랜덤 추측, 수직선 = 10% 교차점).*
