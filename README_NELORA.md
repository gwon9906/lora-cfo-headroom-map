...
>
> **그들 신경망을 같은 배치에서 돌렸다(§5-4): 표준 수신기가 DNN 을 +2.08 dB
> (그들 macro 평균으로는 +2.24 dB) 앞선다.** 그 DNN 은 그들 코드로 100에포크 재학습한
> 것이고, macro 이득 +1.50 dB 로 **그들이 발표한 SF7 수치 +1.46 dB 를 재현한다.**
...
| **같은 배치에서 DNN 과 비교** | **확정.** 표준 RX − DNN = **+2.08 dB** (micro) / **+2.24 dB** (macro) (§5-4) |
| 그 DNN 이 제대로 학습된 것인가 | **재학습함.** 100에포크 후 macro 이득 +1.50 dB = 그들 발표 +1.46 dB (§5-6) |
...
> 그들 baseline 을 +3.46 dB 앞선다. 같은 배치에서 그들 신경망은 +1.39 dB 를 얻는다
> (100에포크 재학습 후, 그들 발표 수치 수준).**
>
