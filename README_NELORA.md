...
>
> **그리고 그들 신경망을 같은 배치에서 돌렸다(§5-4): 표준 수신기가 DNN 을 +2.96 dB
> (그들 macro 평균으로는 +3.14 dB) 앞선다.**
>
...
| **같은 배치에서 DNN 과 비교** | **측정 완료.** 표준 RX − DNN = **+2.96 dB** (micro) / **+3.14 dB** (macro) (§5-4) |
...
> NELoRa 의 신경망보다 큰 이득을 낸다.**
>
> 같은 배치에서 측정했다(§5-4). 단, 공개 체크포인트는 저자들이 "trained for a limited
> amount of time" 이라고 밝힌 것이라 DNN 쪽은 **하한**이다. 그들이 **발표한** SF7 이득
> (+1.46 dB) 을 대신 써도 결론은 같다.
...
### 5-4. 같은 배치에서 DNN 을 돌렸다

`nelora_dnn_eval.py` 를 그들 저장소 안에서 돌렸다. 데이터 로딩·필터·잡음·SNR 축이
전부 그들 코드이고, 배치 루프 안에 표준 RX 팔 하나만 더했다.
SF7, 공개 체크포인트(`checkpoint/sf7/100000_{maskCNN,C_XtoY}.pkl`), 평가 3000심볼,
필터 통과 13,714/14,352 (95.55%).

**10% SER 도달 SNR (같은 심볼·같은 잡음):**

| 검출기 | micro | macro (그들 방식) |
|---|---|---|
| `decode_loraphy` (U=100) | −15.90 | −15.43 |
| NELoRa DNN | −16.14 | −15.93 |
| **표준 RX (학습·정렬 없음)** | **−19.10** | **−19.06** |

| baseline 대비 이득 | 10% | 20% | 30% |
|---|---|---|---|
| NELoRa DNN (micro) | +0.24 | **−0.41** | **−0.75** |
| 표준 RX (micro) | +3.20 | +2.79 | +2.62 |
| NELoRa DNN (macro) | +0.50 | **−0.45** | **−0.93** |
| 표준 RX (macro) | +3.63 | +2.95 | +2.72 |

> **표준 RX − DNN = +2.96 dB (micro) / +3.14 dB (macro).**
> micro 로 읽든 macro 로 읽든 결론이 같다.

### 5-5. 공개 체크포인트는 약하다 — 그래도 결론은 안 바뀐다

이 측정에서 DNN 이 얻은 것은 +0.24~0.50 dB 로, 그들이 **발표한** SF7 이득
(+1.46 dB, §5-3)에도, 초록의 1.84~2.35 에도 못 미친다. 저자들이 README 에
*"these checkpoints are only trained for a limited amount of time and can be improved"*
라고 밝힌 그대로다. **따라서 이 수치는 DNN 의 하한이며, "이득이 재현되지 않는다" 는
주장의 근거로 쓰면 안 된다.**

결론이 체크포인트 품질에 의존하지 않는다는 것이 중요하다:

| DNN 쪽에 무엇을 쓰든 | 표준 RX 와의 차이 |
|---|---|
| 이번에 측정한 공개 체크포인트 (+0.24 dB) | **+2.96 dB** |
| 그들이 발표한 SF7 이득 (+1.46 dB) | **+1.74 dB** |
| 초록 범위의 최대값 (+2.35 dB) | **+0.85 dB** |

세 경우 모두 표준 수신기가 앞선다.

### 5-6. 저SNR 에서 DNN 이 baseline 보다 나빠진다

공개 체크포인트에서 관측된 것으로, 임계 스윕이 필요한 이유를 다시 보여준다
(micro 정확도):

| SNR | baseline | DNN | 표준 RX | DNN − baseline |
|---|---|---|---|---|
| −14 | 94.7% | 99.4% | 99.8% | +4.7%p |
| −16 | 89.7% | 91.9% | 99.3% | +2.2%p |
| **−17** | 82.6% | 78.6% | 98.7% | **−4.0%p** |
| **−18** | 74.1% | 63.0% | 96.9% | **−11.1%p** |
| **−20** | 46.7% | 32.3% | 81.2% | **−14.4%p** |

DNN 이 이기는 구간은 −14~−16 뿐이고, **−17 아래로는 baseline 보다 나쁘다.** 그래서
20/30% 임계에서 DNN 의 "이득" 이 음수가 된다. 10% 하나만 봤다면 +0.24 dB 로 끝났을
것이다. 이것 역시 학습 부족의 징후일 수 있으므로 단정하지 않는다.
...
1. **DNN 은 돌렸다(§5-4). 남은 것은 체크포인트 품질과 held-out 이다.**
   공개 체크포인트는 저자들이 학습 부족이라고 밝힌 것이라 DNN 쪽 수치는 하한이다.
   제대로 하려면 기록된 분할로 **재학습**해야 하고, 그것이 GPU 작업으로 남는다.

   **held-out 은 현재 불가능하다.** `train()` 의 `random_split` 이 torch 시드를 잡지
   않아(모듈 상단 `np.random.seed(10)` 는 torch 에 닿지 않는다) 체크포인트가 어느 10% 를
   안 봤는지 복원할 수 없다. 따라서 §5-4 는 **as-run** 측정이고, DNN 은 평가 심볼의
   ~90% 를 학습에서 봤다. 즉 **DNN 에 유리한 조건**에서 나온 결과다.
