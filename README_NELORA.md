...
>
> ⚠️ 이 문장은 둘을 대체재처럼 놓는다. 함께 쓰면 어떻게 되는지는 §5-9~§5-11 에서 잰다.
> 검출기 쪽은 끝났다(§5-10). 신경망 쪽은 학습이 수렴하지 않아 **아직 결론을 못 쓴다**(§5-11).
...
### 5-11. 대역제한 입력 DNN — 아직 결론을 쓸 수 없다

같은 필터를 준 입력으로 150에포크 재학습한 신경망:

| | 필터 없음 | 대역제한 입력 |
|---|---|---|
| NELoRa DNN (10% SER) | −14.35 | −14.27 (+0.08) |
| baseline 대비 | +1.35 dB [+0.70, +1.97] | **−1.76 dB** [−2.36, −1.22] |
| 표준 RX 와의 차 | −1.97 [−2.59, −1.31] | −2.04 [−2.68, −1.49] |

§5-9 판정표로는 "표준 RX(−16.31) 에도 못 미침" 이고, 대역제한 조건에서는 **자기
baseline 보다 1.76 dB 뒤진다**.

> ⚠️ **그러나 이 팔은 수렴하지 않았다. 지금 수치로 결론을 쓰면 안 된다.**

학습곡선(`results/curve_sf7_{plain,lpf}.csv`, held-out 검증 정확도 @ −17 dB):

| 팔 | 시작 | 최고 | 최종 | 마지막 50에포크 기울기 |
|---|---|---|---|---|
| 필터 없음 | 0.725 | 0.783 (ep 77) | 0.709 | −0.019 %p/에포크 (수렴, 과적합 기미) |
| **대역제한** | 0.723 | **0.834 (ep 144)** | 0.795 | **+0.031 %p/에포크 — 아직 상승 중** |

귀무 결과는 학습을 덜 시켜도 쉽게 나온다. 대역제한 입력은 분포가 달라 적응이 느리고,
150에포크에서 최고점이 144 라는 것은 아직 여유가 있다는 뜻이다. **400에포크로 연장해
평탄해진 뒤에 다시 잰다.**

또 하나, 위 표는 두 팔 모두 **최종 에포크**로 쟀다. 필터 없음 팔은 ep 77 이 최고이고
그 뒤 과적합했으므로, 지금 수치는 그 팔에 불리하다. 재측정 때는 **검증 최고 에포크**로
맞춘다 — 그 선택이 평가에 쓰는 held-out 으로 이뤄지므로 DNN 에 유리한 쪽이며, 우리
주장 방향에서는 보수적이다.
