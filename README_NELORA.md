...
> NELoRa 가 신경망으로 얻은 것보다 큰 이득을 낸다.**
>
> 단, 두 수치는 아직 같은 배치에서 나온 것이 아니다. 확정은 §8-1.
...
1. **신경망을 돌리지 않았다 — 남은 유일한 실질 구멍.**
   체크포인트는 데이터셋과 같은 [Google Drive](https://drive.google.com/drive/folders/12o3kqfBGrWG2YWegBa-sqErpAUsmLIOO)
   에 있다(`checkpoint/sf{sf}/100000_{maskCNN,C_XtoY}.pkl`). 학습 불필요, 추론만.
   `python main.py --sf 7 --batch_size 128`.

   설계는 **그들 `test()` 루프 안에 팔을 하나 추가**하는 것이다. 표준 RX 를 별도
   스크립트로 돌리면 로딩·필터·SNR 축이 미세하게 갈리고 그 차이가 3 dB 안에 섞인다.

   ```python
   # test() 의 배치 루프 안, DNN 예측을 얻는 바로 그 자리
   pred_dnn = est_code                   # 그들 코드 그대로
   pred_std = standard_rx(dataX_test)    # 같은 배치, 같은 텐서
   ```

   함정 셋: (a) 조건을 바꿀 때마다 `pkl_{sf}.pkl` 캐시를 지운다(README 명시),
   (b) 손대기 전에 그들 `test()` 를 그대로 돌려 논문 곡선이 나오는지 먼저 확인한다,
   (c) 공개 체크포인트는 "trained for a limited amount of time" 이라 **하한**이다.

   그리고 **두 번 돌린다** — as-run(전량 평가, 그들 수치 재현용)과 held-out(10% 만,
   가능하면 패킷 단위). 두 값의 차이가 §1-4 `test()` 주장의 근거가 된다. 차이가 없으면
   그 항목을 내린다.

   목표는 같은 배치에서 나온 이 한 줄이다:
