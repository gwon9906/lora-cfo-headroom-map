...
>
> **그들 신경망을 같은 배치에서 돌렸다(§5-4): 표준 수신기가 DNN 을 +2.61 dB
> (그들 macro 평균으로는 +3.22 dB) 앞선다.** 하네스는 그들 `test()` 를 무수정으로
> 돌린 값과 네 지점 모두 **0.1 dB 이내**로 일치한다.
>
...
| **같은 배치에서 DNN 과 비교** | **확정.** 표준 RX − DNN = **+2.61 dB** (micro) / **+3.22 dB** (macro) (§5-4) |
...
> 그들 baseline 을 +3.46 dB 앞선다. 같은 배치에서 그들 신경망은 +0.85 dB 를 얻는다.**
>
> 공개 체크포인트는 저자들이 학습 부족이라 밝힌 것이라 DNN 쪽은 하한이다. 그들이
> **발표한** SF7 이득(+1.46 dB)을 대신 넣어도 표준 수신기가 2.0 dB 앞선다.
...
| NELoRa DNN | −13.96 (**+0.85**) | −13.30 (**+0.52**) |
| **표준 RX (학습·정렬 없음)** | **−16.57 (+3.46)** | **−16.53 (+3.74)** |

> **표준 RX − DNN = +2.61 dB (micro) / +3.22 dB (macro).**
...
### 5-6. 체크포인트 품질과 무관하게 성립하는가

공개 체크포인트의 +0.85 dB 는 그들이 **발표한** SF7 이득 +1.46 dB(§5-3)에 못 미친다.
학습 부족 때문이므로 **하한**으로만 읽는다. 결론이 그 품질에 걸리지 않는지 본다:

| DNN 쪽에 무엇을 쓰든 | 표준 RX(+3.46) 와의 차이 |
|---|---|
| 이번에 측정한 공개 체크포인트 (+0.85) | **+2.61 dB** |
| 그들이 발표한 SF7 이득 (+1.46) | **+2.00 dB** |
| 초록 범위의 최대값 (+2.35) | **+1.11 dB** |

⚠️ 아래 두 줄은 **다른 baseline·다른 데이터셋** 기준이다(§5-3). 엄밀한 뺄셈이 아니라
**범위 감각**으로만 쓸 것. 엄밀한 값은 첫 줄뿐이다.
...
1. **DNN 은 돌렸고 하네스도 검증됐다(§5-4). 남은 것은 체크포인트 품질이다.**
   공개 체크포인트는 그들 발표 SF7 곡선(+1.46 dB)에 못 미치는 +0.85 dB 다. 저자들이
   학습 부족이라 밝힌 그대로이므로, 논문 수치와 맞추려면 **기록된 분할로 재학습**해야
   한다. 그것이 남은 GPU 작업이고, 동시에 held-out 문제도 같이 풀린다.

   **held-out 은 재학습 없이는 불가능하다.** `train()` 의 `random_split` 이 torch 시드를
   잡지 않아(모듈 상단 `np.random.seed(10)` 는 torch 에 닿지 않는다) 체크포인트가 어느
   10% 를 안 봤는지 복원할 수 없다. §5-4 는 as-run 이고 DNN 이 평가 심볼의 ~90% 를
   학습에서 본 상태, 즉 **DNN 에 유리한 조건**이다.

   그래도 차이가 남으면 그들 평가 행렬(`sf7_v1_7_125000.mat`)을 재현한다. MATLAB 방식으로
   per-SNR 데이터셋을 만들어 그들 경로를 돌리는 것이라 오래 걸리지만 결정적이다.
